# Quote Arena — Convex All Gas Hackathon build plan

> **One line:** Post a home-repair job once; the app finds local contractors, emails them all with your photos, and turns replies into a live, normalized quote leaderboard.
>
> **Repo:** `hack-gas3` · **AgentMail account:** A · **Event:** Convex All Gas Hackathon · **Deadline:** September 22, 2026, 12:00 PM PT · **Stack:** Convex (backend + static hosting on `*.convex.site`), React + Vite + Tailwind, any OpenAI-compatible LLM endpoint (env-configured; described as "OpenAI" in judge-facing docs), Firecrawl, AgentMail.
>
...
## 3. Product spec — Quote Arena (get contractor quotes without the phone tag)

### 3.1 The pitch
Getting three quotes for a fence, a roof leak, or a bathroom reno means finding contractors, writing the same email eight times, and then decoding eight replies that all price things differently. **Quote Arena** posts the job once, finds local contractors, emails them all from one inbox with your photos, and turns every reply into a normalized quote card on a live leaderboard — price, timeline, what's included, what's missing — and can answer the contractors' clarifying questions for you. Emotional payoff: relief and control; the fun payoff is watching the leaderboard reorder live when a better quote lands.

### 3.2 Core loop
Homeowner posts a job (trade, description, photos, city/postcode, timing, budget band) → **Firecrawl** `search` finds local contractors, `scrape` their sites and the **LLM** extracts contact/service/area info → the user (or auto) selects who to contact → **AgentMail** creates a job inbox and sends an RFQ with photos to each → replies via webhook → LLM extracts a normalized quote `{priceLow, priceHigh, currency, timelineDays, startEarliest, includes[], excludes[], questions[], confidence}` → leaderboard reorders live → contractor questions are answered by an **Agent** (using the job details) with a human-approval toggle → **cron** nudges non-responders after 48h → household members share the board live.

### 3.3 Data model
- `jobs`: ownerId, slug, trade, title, description, photoIds[], city, region, timing, budgetBand?, caseCode (short unique code for email routing, see §4.3), status (`draft|sourcing|collecting|decided`), autoReply (bool), createdAt. Index by ownerId, slug, caseCode.
- `jobMembers`: jobId, userId.
- `contractors`: jobId, name, website, email?, phone?, services[], serviceArea?, ratingSnippet?, sourceUrl, rawExcerpt, selected (bool), rfqStatus (`none|queued|sent|delivered|bounced|replied|declined`), sentMessageId?, sentAt?, lastNudgeAt?. Index by jobId, by jobId+rfqStatus.
- `quotes`: jobId, contractorId, messageId, priceLow?, priceHigh?, currency, timelineDays?, startEarliest?, includes[], excludes[], notes?, confidence, model, rank? (computed in query), createdAt, supersededBy?. Index by jobId, by contractorId.
- `messages`: jobId, contractorId?, direction, messageId, threadId, subject, extractedText, fullText, classification (`quote|question|decline|auto_reply|other`)?, receivedAt. Index by messageId, by jobId, by threadId.
- `questions`: jobId, contractorId, messageId, question, proposedAnswer, status (`pending_approval|sent|dismissed`), model. Index by jobId+status.

### 3.4 Convex functions
- `jobs.create` (schedules `sourcing.discover`), `jobs.get`, `jobs.listMine`, `jobs.setAutoReply`, `jobs.decide` (marks winner; emails polite "we went another way" to the others via `sendMail`).
- `sourcing.discover` (internalAction): Firecrawl `search("<trade> contractor <city>")` ×2 query variants, limit ~10, `scrapeOptions.formats: ["markdown"]`; for each result run `extract()` → contractor row (skip directories/aggregators via LLM flag `isDirectory`). Optionally `scrape` the contact page for an email if the homepage has none.
- `contractors.addManual` (mutation) — for the demo and for contractors the user already knows.
- `rfq.send` (internalAction): builds the RFQ (LLM-drafted from job; photos as public storage URLs + attachments) and `sendMail()`s to every selected contractor; sets status.
- `http.ts` webhook → `mail.ingest` → `ai.classifyInbound` → if `quote`, `extract()` quote row (supersede older quote from same contractor); if `question`, create `questions` with a proposed answer from the Agent; if `autoReply` on, send it immediately.
- `quotes.leaderboard` (query): ranks by a transparent score (price mid × timeline weight, missing-info penalty) — show the formula in a tooltip.
- `crons.ts`: every 6h `nudges.sweep` — contractors `sent` > 48h with no reply get one follow-up (max 1).
- Agent component: per-contractor thread; instructions = "answer only from the job facts; if unknown, ask the homeowner".

### 3.5 Integration specifics
- **Firecrawl:** discovery via `search`; site `scrape` for contact details; keep `sourceUrl` + excerpt for the "how we found them" hover.
- **LLM:** contractor extraction; RFQ drafting; quote normalization with explicit `null` for missing fields (the UI shows "not stated" chips — this is the product's value); question answering via Agent.
- **AgentMail:** single app inbox (§4.3), `[QA-xxxx]` job code in subjects; RFQ with attachments; replies with `reply()` to keep threads; delivery/bounce events update `rfqStatus`.

### 3.6 UI
- **/new** job form with photo upload; trade select; live "we'll contact contractors in <city>" hint.
- **/j/:id** the arena: top = status strip (Found 9 · Contacted 7 · Replied 3); left = contractor list with source links and checkboxes (before sending) → status pills (after); center = **leaderboard** of quote cards (rank badge, price range bar, timeline, includes/excludes chips, "not stated" chips highlighted, "reply" button) that **animate reorder** (use a FLIP/layout animation lib like `motion` or `@formkit/auto-animate` — install with pnpm, no version pin); right = thread drawer + pending questions with *Approve & send*.
- **Compare view**: pick up to 3 quotes → side-by-side table.
- Playful but clean: sports-bracket energy, big rank numbers, confetti only when the user picks a winner.

### 3.7 Demo storyboard (≤ 3:00)
1. (0:00) "Our back fence fell down. Watch me get quotes without making a single call." Post the job with 2 photos. (0:25)
2. Contractors appear as Firecrawl finds them — hover one to show the source page. Add one manual contractor (your own email). Select 5, *Send requests*. (1:00)
3. Show one sent RFQ (photos attached). Board shows *Contacted 5*. (1:15)
4. Phone: reply as a contractor "We can do it for $3,200–3,600, cedar, 2 weeks out, includes removal, excludes staining." Card appears, ranked #1 with chips. Reply from a second seeded contractor already in place; now reply again with "$2,900 all in, next week" → the new card animates to #1. (2:15)
5. A contractor asks "Is the ground level?" → question card with a proposed answer from the job details; *Approve & send*. Show compare view, pick a winner, others get the courtesy email. URL on screen. (2:55)

### 3.8 Fixtures & test plan
Seed a job with 3 seeded contractors (all pointing at the human's own address via override) and 2 quotes in place; test the supersede logic (same contractor sends a revised quote) and bounce handling.

### 3.9 Stretch
Price sanity check against a Firecrawl-scraped cost guide (e.g. "typical fence cost in <region>") shown as a band on the price bar; calendar invite for site visits; SMS-free "I'm on my way" replies.

### 3.10 Risks
Contractor websites without emails — fall back to "phone only" chip and let the user add the email. Never email real businesses from dev (override). Keep the ranking formula visible so it doesn't look arbitrary.
